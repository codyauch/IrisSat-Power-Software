...
try:
    ...
    subprocess.run(f'python waf2 configure --toolchain=msp430-elf- --endian=little --enable-if-can --with-os=freertos --includes="{INCLUDES}" -v', shell=True, check=True)
    print("building...")
    # subprocess.run('python waf build -v', shell=True, check=True)
    subprocess.run('python waf2 build -v', shell=True, check=True)
